...
def main():
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        if args.finetune:
            # Fine-tune mode: only load model weights, reset everything else
            print(f"  Fine-tune mode: loaded model weights only (prev best: {ckpt.get('best_val_acc', 0)*100:.2f}%)")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Validate
        val_loss, val_acc, val_top5 = validate(model, val_loader, device)
