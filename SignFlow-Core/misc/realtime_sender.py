"""
Webcam ASL prediction + IPC sender for SignFlow overlay.

Captures webcam, runs the 3D Landmark Transformer for sign prediction,
builds a sentence character-by-character, and sends it to the overlay
via QLocalSocket IPC.
...
def main():
    ...
    while True:
        ...
        if frame_idx % PREDICT_INTERVAL == 0 and len(hand_buffer) >= MIN_BUFFER:
            ...
            if top_conf >= PREDICTION_THRESHOLD:
                detected_label = class_names.get(top_idx, str(top_idx))
        ...
        if detected_label:
            ...
            if (candidate_stable_frames >= MIN_STABLE_FRAMES_FOR_APPEND
                ...
                if current_sentence != last_sent_sentence and send_caption(current_sentence.strip()):
                    last_sent_sentence = current_sentence
                    print(f"\r  Sent: {current_sentence.strip()}" + " " * 20, end="", flush=True)
        else:
